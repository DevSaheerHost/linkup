-- Discovery surfaces (For You / Reels) shouldn't re-show a post the viewer
-- already saw. postviews has a unique (post_id, user_id) row per viewer/post
-- pair with created_at = first-seen time (see registerView() in app.js,
-- which upserts with ignoreDuplicates), so "seen within N days" is a simple
-- existence check. Cooldown window: 60 days ("2 months"). Following stays
-- untouched - it's meant to always show the latest from people you follow,
-- seen or not.
