-- The hard "not exists postviews" filter from the previous migration
-- backfires on small/new accounts: once a viewer has seen every eligible
-- post, the candidate pool is empty and For You/Reels shows nothing at
-- all - worse than repeating content. Replace the hard exclude with a
-- large score penalty instead: an unseen post always outranks a seen one
-- (penalty dwarfs any realistic combination of the other score terms), so
-- seen posts are still fully deprioritized under normal conditions, but
-- once unseen candidates run out the feed gracefully falls back to
-- showing already-seen posts (best-scoring first) rather than going empty.
