-- Wires normalize_tags() into topic-affinity matching (hashtags + keywords)
-- so near-duplicate tags (developer/development, coding/coder, amartphone/
-- smartphone typo) contribute to the same affinity bucket, and adds a
-- modest score boost for verified/official accounts (still subject to the
-- seen-post penalty and recency decay - a nudge, not a guarantee).
